...
union
{
	...
	void(Server::*MemberProc)(SOCKET s);
}pppro;
...
void Server::recvMessage(SOCKET socket){
	unique_lock<mutex>locker(this->r, defer_lock);
	unique_lock<mutex>locker2(this->r2, defer_lock);
	unique_lock<mutex>locker3(this->r3, defer_lock);
	unique_lock<mutex>locker4(this->r4, defer_lock);
	unique_lock<mutex>locker7(this->r7, defer_lock);
	unique_lock<mutex>locker9(this->r9, defer_lock);



	locker.lock();							 //recvBuf为临界资源，需要上锁
	...
	if(receivedBytes == SOCKET_ERROR || receivedBytes == 0){    //接受数据错误，把产生错误的会话socekt加入sessionsClosed队列
		...
		string status;//状态
		string sed;//发回的响应报文
		string IF_OK = "200";//文件是否请求到 缺省为200
		string W_OK = "OK";//状态说明 缺省为OK
		string content_type;

	    cout <<"\r\n"<< recvBuf;
		locker2.lock();				 //recvBuf为临界资源，需要上锁
		int pos = 0;
		while (recvBuf[pos] != ' ')
		{
			pos++;
		...
		while (recvBuf[pos] != ' ')
		{
			url += recvBuf[pos];//读取请求内容
			pos++;
		}
		pos++;
		while (recvBuf[pos] != '\r')
		{
			http += recvBuf[pos];//读取版本
			pos++;
		}
		pos += 2;//跳过\r\n
		while (recvBuf[pos] != '\r')//跳过host行
		{
			pos++;
		}
		pos += 2;//跳过\r\n
		...
		pos += 1;
		int pos_acc = rec.find("Accept: ");
		pos_acc+=8;
		while (rec[pos_acc] != '\r')
		{
			content_type += rec[pos_acc];
			pos_acc += 1;
		}

		string file_s = "D:/resume" + url;
		char*filename = (char*)file_s.data();
		std::ifstream i(filename,ios::binary);
		if (i.fail())
		{
			IF_OK = "404";
			W_OK = "NOT FOUND";
			cout << "请求资源" << url << "未找到，请检查" << file_s << "是否为合法路径"<<endl;
		}
		else
		{
			cout << "资源" << url << "请求成功"<<endl;
		}
		std::string file((std::istreambuf_iterator<char>(i)), std::istreambuf_iterator<char>());
		int len = file.length();
		char length[100];
		itoa(len, length, 10);
		string temp_out = http + ' ' + IF_OK + ' ' + W_OK + "\r\n" + "Connection: " + status + "\r\n" + "Content-Length: " + length + "\r\n" + "Content-Type: " + content_type + "\r\n" + "\r\n";
		sed = temp_out + file;
		locker2.unlock();
		cout << "\r\n当前发送的响应报文如下："<<endl;
		cout << temp_out<<"实体主体部分略"<<endl;
		locker3.lock();
		this->sendMessage(socket, sed);
		locker3.unlock();
		locker4.lock();                          //recvBuf为临界资源，需要上锁
...
void Server::sendMessage(SOCKET socket,string msg){
	...
	if(rtn == SOCKET_ERROR){					//发送数据错误，把产生错误的会话socekt加入sessionsClosed队列
		locker9.lock();									//Session队列为临界资源，需要上锁
		this->AddClosedSession(socket);
		locker9.unlock();
	}
}
			...
			for(list<string>::iterator msgItor = this->rcvedMessages->begin();msgItor != this->rcvedMessages->end();msgItor++){//对消息队列中的每一条消息
				for(list<SOCKET>::iterator sockItor = this->sessions->begin();sockItor != this->sessions->end();sockItor++){//对会话socket队列中的每个socket
					if (FD_ISSET(*sockItor, &this->wfds)) {
						...
						char length[10000];
						itoa(len,length,10);
						sed = http + ' ' + "200" + ' ' + "OK" + "\r\n" + "Connection: " + status + "\r\n" + "Content-Length: " + length + "Content-Type: " + "text/html"+"\r\n"+"\r\n"+file;
						this->sendMessage(*sockItor,sed);
					}
				}
			}
		}
...
void Server::ReceieveMessageFromClients(){
	lock_guard<mutex> lockGuard(this->r5);
	if(this->numOfSocketSignaled > 0){
		...
		for(list<SOCKET>::iterator itor = this->sessions->begin();itor!=this->sessions->end();itor++){
			if (*itor != INVALID_SOCKET) {
				if (FD_ISSET(*itor, &rfds)) {  //某会话socket有数据到来
					...
					getpeername(*itor, (struct sockaddr*)&sockAddr, &iLen);
					cout <<"来自"<< inet_ntoa(sockAddr.sin_addr)<<"的客户端的";
					cout << ntohs(sockAddr.sin_port)<<"端口发来请求"<<endl;
					thread t(&Server::recvMessage, std::ref(*this), *itor);
					t.detach();
				}
			}
		}//end for
...
int Server::Loop(){
	...
	//添加部分
	SOCKET newSession;
	pppro.MemberProc = &Server::recvMessage;
